...
export const selectMultiple = (
  ...
  return [
    ...parentNode.querySelectorAll(
      selectors.reduce((prev, current) => prev + ", " + current)
    ),
  ];
};
...
export const selectSingle = (
  ...
  return parentNode.querySelector(
    selectors.reduce((prev, current) => prev + ", " + current)
  );
};
...
export const objectSelector = <Type extends StringObject>(
  parentNode: Element,
  selectorModel: Type
): Type => {
  ...
  for (const key of Object.keys(selectorModel)) {
    if (typeof selectorModel[key] === "string") {
      ...
      Array.isArray(selectorModel[key]) &&
      selectorModel[key].length === 1
    ) {
      const content = extractText(
        selectMultiple(parentNode, selectorModel[key][0])
      );
      if (content) {
        selectedObject[key] = content;
      }
    } else if ({}.toString.apply(selectorModel[key]) === "[object Object]") {
      const { content } = objectSelector(parentNode, {
        content: selectorModel[key].selector,
      });
